...
var BillRetrievalNamespace = (function () {
	...
	const dataSource = require('../../Shared/RousrData/RousrDataSource').RousrDataSource;
	const billDiffernator = require('../../Shared/RousrData/BillDiffernator').BillDiffernator;
	...
	var rousrDataSource = dataSource.create(billRetrieveConfig);
	...
	BillRetriever.prototype.getCongressMembers = function(next)
	{
	    this.getHouseMembers((houseError) => {
	        ...
	        this.getSenateMembers((senateError) => {
	            ...
	            rousrDataSource.updateMembers(congressMembers, function (error) {
	                if (error) {
	                    return next(error);
	                }
	            });
	        ...
	        if (info.results[0].bills && info.results[0].bills.length > 0) {
	            ...
	            if (currentBills && currentBills.length > 0) {
	                if (billRetrieveConstants.GET_SPECIFIC_BILL_DATA) {
	                    ...
	                    rousrDataSource.updateBills(currentBills, function (err) {
	                        if (err) {
	                            return next(err);
	                        }	                        
	                        else {
	    ...
	    try {
	        ...
	        if(billInfo.status === 'OK') {
	            ...
	            billDiffernator.onBillUpdated(billInfo.results[0], function (updatedBillData) {
	                ...
	                rousrDataSource.updateBills(billInfo.results, function (err) {
	                    if (err) {
	                        return next(err);
	                    }
	                });
